@Maximo-API
   Feature: 
   Close Field Access Notification (FAN) in Maximo
...
Scenario: Close Field Access Notification (FAN)
   ...
   And request read('FAN_Payloads/updateFAN_payload.xml')
   When soap action UPDATEFANEndPoint
   Then status 200

   #Query SR for FAN verification with Deactivate
   Given url GBSSREndPoint
   And request read('FAN_Payloads/queryFAN_payload.xml')
   When soap action GBSSREndPoint
   Then status 200
